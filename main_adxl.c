...
//GYRO reg values
#define MEASURE_MODE 0b00001000
#define FULL_RES 0b00001111
...
void write_reg(int reg_addr,uint8_t data){
        uint8_t src[2]={0}; 
        src[0]=reg_addr;
        src[1]=data;
        int ret=i2c_write_blocking(I2C,ADXL_ADDR,src,2,false);
        if(ret<0)puts("write error");
}

void read_reg(int reg_addr,uint8_t *data,size_t sz){
        uint8_t src[1]={0}; 
        src[0]=reg_addr;
        int ret=i2c_write_blocking(I2C,ADXL_ADDR,src,1,false);
        if(ret<0)puts("read write error");

        ret=i2c_read_blocking(i2c0,ADXL_ADDR,data,sz,false);
        if(ret<0)puts("read error");
}
...
void setup(){
    ...
    //gyro init
    write_reg(POWER_CTL,MEASURE_MODE);
    write_reg(DATA_FORMAT,FULL_RES);
...
int main(){
	...
	while(1){
		...
		*/
	        int16_t data[3];
                read_reg(X_AXIS,data,6);
                double x=4*data[0]/(double)8/(double)1000;
                double y=4*data[1]/(double)8/(double)1000;
                double z=4*data[2]/(double)8/(double)1000;
                printf("x:%f\n",x);
                printf("y:%f\n",y);
                printf("z:%f\n",z);
		printf("pitch:%f\n",pitch(x,y,z));
