...
void set_pwm(motor mot,unsigned int pwm){
...
void turn_motor_off(motor* mot,int sz){
	for(int i=0;i<sz;i++){
		...
		pwm_set_gpio_level(mot[i].in1,0);
		sleep_ms(1);
		gpio_put(mot[i].in1,0);
		sleep_ms(1);
		gpio_put(mot[i].in1,0);
	}
}
...
int main(){
	...
	while(1){
		if(is_command==1){
		    printf("is com\n");
		    is_command=0;
		    exec_command(motors,4);
		}
		for(int i=0;i<4;i++){
			int in1=gpio_get(motors[i].in1);
			int in2=gpio_get(motors[i].in2);
			printf("motor %d stat:in1:%d in2:%d\n",i,in1,in2);
		}
			printf("\n");
